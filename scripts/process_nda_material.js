...
const PDF_FILE = 'nda_material.pdf';
const STATE_FILE = 'scratch/nda_extraction_state.json';
...
async function main() {
    ...
    let state = { currentChunk: 1, totalChunks: 35, extractedQuestions: [] }; // ~35 chunks for a 1314 page book
    ...
    for (let i = state.currentChunk; i <= state.totalChunks; i++) {
        ...
        const prompt = `You are an expert NDA exam content creator.
I have attached a massive NDA preparation textbook. 
...
function injectIntoDataJs(questions) {
    ...
    // NDA Rule: Math goes to NDA Mathematics. Non-math goes everywhere else.
    let mathExams = db.filter(e => e.title.startsWith('NDA Mathematics'));
    ...
    questions.forEach((q, idx) => {
        ...
        q.explanation = q.explanation + "\n\n(Extracted from new NDA Material)";
    ...
    console.log(`Successfully injected ${mathCount} Maths (to NDA) and ${nonMathCount} Non-Maths (to All others).`);
}
