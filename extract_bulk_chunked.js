...
const directories = ['AFCAT Papers', 'PYQ Papers', 'NDA Papers'];
const UUID_REGEX = /^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}/i;

let allQuestions = { gs: [], english: [], maths: [], afcat: [] };

let state = { processedFiles: [] };
if (fs.existsSync('extraction_state.json')) {
    state = JSON.parse(fs.readFileSync('extraction_state.json', 'utf8'));
}
    ...
    for (const dir of directories) {
        ...
        for (const f of files) {
            if (f.toLowerCase().endsWith('.pdf') && !UUID_REGEX.test(f) && !f.includes("part")) {
                ...
                else if (filepath.toLowerCase().includes('gat') || filepath.toLowerCase().includes('general-ability')) { subject = 'gs'; maxQ = 150; }
...
[
  {
    ...
    "exam": "The designated exam (e.g., NDA, CDS, AFCAT)",
    "subject": "The specific subject (e.g., Mathematics, General Ability Test, English, General Knowledge)"
  }
]
...
Do NOT truncate the JSON. If the PDF doesn't have the questions requested, just output an empty array [].`;
        ...
        if (fileExtractedQuestions.length > 0) {
            ...
            fs.writeFileSync('extraction_state.json', JSON.stringify(state, null, 2), 'utf8');
            console.log(`\nSuccessfully saved state. Total accumulated: GS=${allQuestions.gs.length}, Eng=${allQuestions.english.length}, Maths=${allQuestions.maths.length}, AFCAT=${allQuestions.afcat.length}`);
        } else {
