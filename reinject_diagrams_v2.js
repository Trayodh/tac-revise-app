...
const FOLDER_MAP = {
    ...
    'reasoning afcat': ['afcat-verbal-reasoning'] // basic fallback
};

let injectedCount = 0;

for (const imgPath of imageFilesAbsolute) {
    // RAW relative path for matching folders
    const rawRelativePath = path.relative(__dirname, imgPath).replace(/\\/g, '/');
    const basename = path.basename(imgPath);
    const basenameWithoutExt = path.basename(imgPath, path.extname(imgPath));
    
    // Encoded relative path for src attribute
    const encodedRelativePath = rawRelativePath.split('/').map(p => encodeURIComponent(p)).join('/');
    
    let topicId = EXACT_MAP[basename];
    
    if (!topicId) {
        // Fallback to FOLDER_MAP using RAW path
        const parts = rawRelativePath.toLowerCase().split('/');
        ...
        if (parts.includes('polity-20260731t070614z-1-001') || parts.includes('polity')) targetTopics = FOLDER_MAP['polity'];
        if (parts.includes('reasoning afcat-20260731t070612z-1-001') || parts.includes('reasoning afcat')) targetTopics = FOLDER_MAP['reasoning afcat'];
        
        if (targetTopics && targetTopics.length > 0) {
            topicId = targetTopics[0];
        }
    }
    ...
    for (const [file, content] of Object.entries(dbContents)) {
        if (content.includes(`src='${encodedRelativePath}'`) || content.includes(`src="${encodedRelativePath}"`)) {
            injected = true;
            break; // already injected
        }
...
console.log(`Successfully injected ${injectedCount} additional diagrams.`);
