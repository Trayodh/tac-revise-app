...
async function researchAndFillGaps(gapFile) {
    ...
    let researchedKnowledge = {};
    if (fs.existsSync('researched_facts_phase3.json')) {
        try {
            researchedKnowledge = JSON.parse(fs.readFileSync('researched_facts_phase3.json', 'utf8'));
            console.log("Resuming from existing researched_facts_phase3.json...");
        } catch(e) {}
    }

    for (const subject of Object.keys(gapsReport)) {
        if (!researchedKnowledge[subject]) researchedKnowledge[subject] = {};
        for (const chapter of Object.keys(gapsReport[subject])) {
            if (!researchedKnowledge[subject][chapter]) researchedKnowledge[subject][chapter] = {};
            for (const topic of Object.keys(gapsReport[subject][chapter])) {
                if (!researchedKnowledge[subject][chapter][topic]) researchedKnowledge[subject][chapter][topic] = {};
                ...
                for (const gap of microTopics) {
                    ...
                    // Skip if already researched (resuming)
                    if (researchedKnowledge[subject][chapter][topic][gap.micro_topic]) {
                        console.log(`[STEP 3] Skipping already researched gap: ${gap.micro_topic}`);
                        continue;
                    }
                    ...
                    try {
                        ...
                        console.error(`[STEP 3] Failed to research gap ${gap.micro_topic}:`, err.message);
                        researchedKnowledge[subject][chapter][topic][gap.micro_topic] = {
                            priority: gap.priority,
                            target_depth: gap.target_depth,
                            structured_content: "Failed to generate due to rate limits.",
                            date_researched: new Date().toISOString()
                        };
                    }
                    
                    // Incrementally save progress to avoid losing data on crash
    ...
    // Final save (already saving incrementally, but just in case)
    fs.writeFileSync('researched_facts_phase3.json', JSON.stringify(researchedKnowledge, null, 2));
