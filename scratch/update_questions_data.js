...
const questionsDataPath = 'questions_data.js';
let content = fs.readFileSync(questionsDataPath, 'utf8');

// Parse CBT_EXAMS_DATABASE
const m = content.match(/const CBT_EXAMS_DATABASE = (\[[\s\S]*?\n\]);/);
if (!m) {
    console.error('Could not parse CBT_EXAMS_DATABASE from questions_data.js');
    process.exit(1);
}
...
const updatedCbtExamsStr = JSON.stringify(exams, null, 2);
const newContent = content.substring(0, m.index) + 'const CBT_EXAMS_DATABASE = ' + updatedCbtExamsStr + ';\n' + content.substring(m.index + m[0].length);
fs.writeFileSync(questionsDataPath, newContent);
console.log(`Updated ${fixedCount} generic questions in questions_data.js to specific chapters!`);
