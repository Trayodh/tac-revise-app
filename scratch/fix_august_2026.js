...
// Find the August 2026 section and replace its array
const augStart = dbText.indexOf('"August 2026": [');
if (augStart !== -1) {
    let augEnd = dbText.indexOf('],', augStart);
    if (augEnd === -1) {
        augEnd = dbText.indexOf(']', augStart);
    }
    const replacement = '"August 2026": ' + JSON.stringify(august2026News, null, 4);
    dbText = dbText.substring(0, augStart) + replacement + dbText.substring(augEnd + 1);
    fs.writeFileSync('current_affairs_db.js', dbText, 'utf8');
    console.log("Successfully replaced August 2026 dummy data with real news.");
} else {
    console.log("Could not find August 2026 array in DB.");
}
