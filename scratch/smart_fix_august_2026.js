...
try {
    ...
    if (dbObj['August 2026']) {
        // Only keep the first 5 items from August 2026, and merge our new data into them
        // so that we retain the advanced fields like upscHighlights
        const newArray = [];
        for (let i = 0; i < august2026News.length; i++) {
            let existingItem = dbObj['August 2026'][i] || {};
            // Overwrite basic fields with new data
            let mergedItem = Object.assign({}, existingItem, august2026News[i]);
            newArray.push(mergedItem);
        }
        dbObj['August 2026'] = newArray;
        
        // Write it back
        const newDbText = prefix + JSON.stringify(dbObj, null, 2) + ';\n';
        fs.writeFileSync('current_affairs_db.js', newDbText, 'utf8');
        console.log("Successfully parsed and updated August 2026 data intelligently.");
    } else {
