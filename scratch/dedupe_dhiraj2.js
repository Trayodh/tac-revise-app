...
// Find the start of the array for July 2026
const julyStartStr = '"July 2026": [';
const startIndex = code.indexOf(julyStartStr);
if (startIndex === -1) {
  console.log("Could not find July 2026");
  process.exit(1);
}

// Find the end of the July 2026 array
// Since "June 2026" follows it
...
if (endIndex === -1) {
  console.log("Could not find end of July 2026 array");
  process.exit(1);
}

// The string representing the array is from startIndex + '"July 2026": '.length to endIndex + 1.
let arrayStr = code.substring(startIndex + '"July 2026": '.length, endIndex + 1);

// arrayStr should be a valid JSON array or at least a JS array.
let julyArray;
try {
  julyArray = eval('(' + arrayStr + ')');
} catch (e) {
  console.log("Failed to eval array:", e);
  process.exit(1);
}

console.log("Total July items before:", julyArray.length);
let nonDhiraj = julyArray.filter(item => !(item.text && item.text.includes('Dhiraj Seth')));
...
// Format as string
let newArrayStr = JSON.stringify(nonDhiraj, null, 2);
// Fix indentation to match the rest of the file
newArrayStr = newArrayStr.replace(/\n/g, '\n  ');

let newCode = code.substring(0, startIndex + '"July 2026": '.length) + newArrayStr + code.substring(endIndex + 1);
