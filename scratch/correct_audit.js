...
for (let i = 0; i < hits.length; i++) {
  ...
  // Check mindmap - count title: (no quotes, JS object notation)
  ...
  // Count both 'title:' and "title:' styles
  const branchCount = (mmStr.match(/title:/g) || []).length;
  if (branchCount < 2) emptyMindmaps.push(id);
}

console.log('Total topics:', total);
console.log('Empty Formulas:', emptyFormulas.length, emptyFormulas.join(', '));
console.log('Empty Mindmaps:', emptyMindmaps.length, emptyMindmaps.join(', '));
