...
for (const [topicId, patch] of Object.entries(PATCHES)) {
  const rx = new RegExp("id:\\s*['\"]" + topicId + "['\"][\\s\\S]{1,4000}?formulas:\\s*[`'\"]([\\s\\S]*?)[`'\"]");
  const match = rx.exec(src);
  if (match) {
    const oldFormulas = match[0];
    const escapedFormulas = patch.formulas.replace(/\`/g, "'");
    const newFormulas = oldFormulas.replace(/formulas:\s*[`'"][\s\S]*?[`'"]/, "formulas: `" + escapedFormulas + "`");
    src = src.replace(oldFormulas, newFormulas);
  }
  
  const rxMm = new RegExp("id:\\s*['\"]" + topicId + "['\"][\\s\\S]{1,4000}?mindmap:\\s*\\{[\\s\\S]*?\\}");
  const matchMm = rxMm.exec(src);
  if (matchMm) {
    const oldMm = matchMm[0];
    const newMmStr = "mindmap: " + JSON.stringify(patch.mindmap, null, 14).replace(/\n/g, '\n              ');
    const newMm = oldMm.replace(/mindmap:\s*\{[\s\S]*?\}/, newMmStr);
    src = src.replace(oldMm, newMm);
    patchesApplied++;
  }
}
