...
function replaceInFile(filePath) {
  ...
  if (startIndex === -1) {
    console.error("Could not find 'July 2026' in " + filePath);
    return;
  }
  
  // Find the end of the array. We look for '  ],' after startIndex.
  // Actually, we can just use regex or find the exact block.
  ...
  if (endIndex === -1) {
      console.error("Could not find end of 'July 2026' array in " + filePath);
      return;
  }
  
  const endReplacement = isLast ? '  ]\n};' : '  ],\n  "June 2026":';
  const newContent = content.substring(0, startIndex - 2) + dataStr + '\n' + endReplacement.substring(4) + content.substring(endIndex + endReplacement.length);
  
  // wait, the substring logic is brittle. Let's just use regex.
  let regex = /"July 2026":\s*\[[\s\S]*?\](,\n\s*"June|\n\};)/;
  content = content.replace(regex, dataStr.substring(0, dataStr.length - 1) + '$1');
  
  fs.writeFileSync(filePath, content, 'utf8');
  console.log("Updated " + filePath);
}

replaceInFile('current_affairs_db.js');
